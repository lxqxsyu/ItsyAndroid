...
import com.guohe.ltsyandroid.manage.http.OkHttpManager;
...
public class FrescoUtils {
     ...
     * @param url
     */
    public static void loadUrl(SimpleDraweeView draweeView, String url){
        loadUrl(draweeView, url, null, 0, 0, null);
    }
     ...
     * @param heightDip
     */
    public static void loadUrl(SimpleDraweeView draweeView, String url, int widthDip, int heightDip){
        loadUrl(draweeView, url, null, DimenUtil.dp2px(widthDip), DimenUtil.dp2px(heightDip), null);
    }
     ...
     * @param listener
     */
    public static void loadUrl(SimpleDraweeView draweeView, String url, BasePostprocessor processor, int width, int height,
                               BaseControllerListener listener){
        if(url == null) url = "";
        load(draweeView, Uri.parse(url), processor, width, height, listener);
    }

    public static void loadFile(SimpleDraweeView draweeView, String file, BasePostprocessor processor, int width, int height,
                               BaseControllerListener listener){
        if(file == null) return;
        load(draweeView, getFileUri(file), processor, width, height, listener);
    }

    public static void loadFile(SimpleDraweeView draweeView, File file, BasePostprocessor processor, int width, int height,
                                BaseControllerListener listener){
        if(file == null) return;
        load(draweeView, getFileUri(file), processor, width, height, listener);
    }

    public static void loadRes(SimpleDraweeView draweeView, int resId, BasePostprocessor processor, int width, int height,
                                BaseControllerListener listener){
        load(draweeView, getResUri(resId), processor, width, height, listener);
    }


    public static void load(SimpleDraweeView draweeView, Uri uri, BasePostprocessor processor, int width, int height,
                                BaseControllerListener listener){
    ...
    public static void init(final Context context,int cacheSizeInM){
        ...
        ImagePipelineConfig config = OkHttpImagePipelineConfigFactory
                .newBuilder(context, OkHttpManager.getInstance().getOkHttpClient())
    ...
    public static PointF getFaceFocusRatio(Bitmap bitmap){
        ...
        LogUtil.e(bitmap.getWidth()+"--width---heitht:"+ bitmap.getHeight()+"---count:"+faceCount);
        if (faceCount == 1){
        ...
        if (pointF != null){
            LogUtil.e("x:"+ pointF.x+"---y:"+pointF.y);
        }
